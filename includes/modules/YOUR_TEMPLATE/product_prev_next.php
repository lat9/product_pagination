...
// BOF - products_pagination - 1 of 8
    $product_names_array[] = $products_ids->fields['products_name'];
// EOF - products_pagination - 1 of 8
  ...
  if (is_array($id_array)) {
    ...
    foreach ($id_array as $key => $value) {
      if ($value == (int)$_GET['products_id']) {
        $position = $counter;
        if ($key == 0) {
// BOF - products_pagination - 2 of 8
          $previous_position = -1;
// EOF - products_pagination - 2 of 8
          $previous = -1; // it was the first to be found
        } else {
// BOF - products_pagination - 3 of 8
          $previous_position = $key - 1;
          $previous = $id_array[$previous_position];
//          $previous = $id_array[$key - 1];
// EOF - products_pagination - 3 of 8
        }
        if (isset($id_array[$key + 1]) && $id_array[$key + 1]) {
// BOF - products_pagination - 4 of 8
          $next_position = $key + 1;
// EOF - products_pagination - 4 of 8
          $next_item = $id_array[$key + 1];
        } else {
// BOF - products_pagination - 5 of 8
          $next_position = 0;
// EOF - products_pagination - 5 of 8
...
// BOF - products_pagination - 6 of 8
//    if ($previous == -1) $previous = $last;      
//      require_once($language_page_directory . (($template_dir != '') ? ($template_dir . '/') : '') . 'products_pagination.php'); -- Removed for v1.2.0
// EOF - products_pagination - 6 of 8
  ...
  // BOF - products_pagination - 7 of 8
  /*
  // EOF - products_pagination - 7 of 8
  ...
  // BOF - products_pagination - 8 of 8
  */
  // EOF - products_pagination - 8 of 8
}
// eof: previous next
?>